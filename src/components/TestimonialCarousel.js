...
export default function TestimonialCarousel({slider}) {
  return (
    <Carousel showThumbs={false} showStatus={false} showIndicators={false} autoPlay={false}  infiniteLoop={true}
    className='w-full max-w-[1024px] flex items-center justify-center'
    >
    {slider.map((item,i)=>{
      const { image, message, name, occupation} = item;
   return(
    <div className='max-w-[840px] mx-auto px-24 text-white'>
